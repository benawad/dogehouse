    ...
    <>
      <div style={divStyle} className="flex flex-row items-center ">
        <div style={avatarStyle} className="flex mr-0.5">
          <SingleUser size="md" src={user.avatarUrl} username={user.username} />
        </div>
        <div className="flex flex-col px-2">
          <span
            className="flex text-primary-100 mr-3 font-bold h-full break-all line-clamp-1 truncate"
            style={testStyle}
          >
            {user.displayName}
          </span>
          <span
            style={{ color: "#5D7290" }}
            className="flex text-primary-300 mr-3"
          >
            @{user.username}
          </span>
        </div>
