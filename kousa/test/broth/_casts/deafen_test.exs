defmodule BrothTest.DeafenTest do
  ...
  describe "the websocket deafen call operation" do
    test "can be used to swap state", t do
      ...
      WsClient.send_call_legacy(t.client_ws, "deafen", %{"value" => true})
      ...
      WsClient.send_call_legacy(t.client_ws, "deafen", %{"value" => false})
    ...
    test "has no effect on initial value", t do
      ...
      # obtain the pseudo-response
      assert_receive({:text, _, _})
      assert %{} == Onion.RoomSession.get(room_id, :deafMap)
    end
  end
end
