defmodule Kousa.GitHubAuth do
  ...
  def get_base_url(conn) do
    with true <- Kousa.Caster.bool(Application.get_env(:kousa, :is_staging)),
         ...
         {:ok, %{"redirect_base_url" => redirect_base_url}} when is_binary(redirect_base_url) <- Poison.decode(json) do
      redirect_base_url
    else
      other ->
        Logger.warn("unknown inbound base_url setting #{inspect other}")
