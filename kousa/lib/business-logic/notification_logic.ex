defmodule Kousa.BL.Notification do
  ...
  def followed(user_you_want_to_follow_id, follower_id) do
    ...
    if not exists do
      case Notifications.insert(%{type: "follow", user_id: user_you_want_to_follow_id, notifier_id: follower_id}) do
        {:ok, notification} ->
          broadcast_notification(user_you_want_to_follow_id, %{notification | notifier: Users.get_profile(notification.notifier_id)})
      end
    end
  end
  ...
  def get_notifications(user_id, cursor) do
    IO.inspect Notifications.get_feed(user_id, cursor)
    Notifications.get_feed(user_id, cursor)
  end
